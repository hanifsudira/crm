...
class ReportController extends Controller
{
    ...
    public function getorderdetail($status,$milestone,$report,$state){
        ...
        if($state=='min'){
            $sql = "SELECT order_num, row_id, product, int_note, TSQ_STATE, TSQ_DESC, DELIVER_STATE, DELIVER_DESC, SEGMENT, CC, SID_NUM, INT_ID 
                    FROM int_report 
    ...
    public function storedetailaction(Request $request){
        ...
        $sby    = $request->sby;
        $note   = $request->note;
        DB::select("INSERT INTO int_report_notes (row_id,fuby,sby,fus_note) VALUES ($rowid,$fuby,$sby,$note) ON DUPLICATE KEY UPDATE fuby=VALUES(fuby),sby=VALUES(sby),fus_note=VALUES(fus_note);");
    }
}
