...
for i, row in enumerate(data):
	...
	TSQ_STATE			= str(row['TSQ_STATE']).replace("'","") #if type(row['TSQ_STATE']) == unicode else str(row['TSQ_STATE'][0][1])
	TSQ_DESC			= str(row['TSQ_DESC']).replace("'","") #if type(row['TSQ_DESC']) == unicode else str(row['TSQ_DESC'][0][1])
