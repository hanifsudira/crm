...
for i,data in enumerate(result):
	...
	FULFILL_STATUS 	= str(data[9])  #-> tidak tampil
	ACC_NAS 		= str(data[10]) 
	NIPNAS 			= str(data[11]) 
	SID_NUM 		= str(data[12]) 
	OH_SEQ 			= str(data[13]) #-> tidak tampil
	MSTONE_SEQ 		= str(data[14]) #-> tidak tampil
	INT_ID 			= str(data[15])
	LI_STATUS_INT 	= str(data[6]) #-> tidak tampil
	MILE_STATUS_INT = str(data[7]) #-> tidak tampil
	SEGMENT			= str(data[16])
